-- ===========================================
-- SYSTEM MAP FEATURE (FIXED)
-- Hierarchical, grounded system documentation and flowcharts
-- This version handles existing enums properly
-- ===========================================

-- ===========================================
-- ENUMS - Add new values to existing enums, create new ones
-- ===========================================

-- Add new values to existing artifact_kind enum
ALTER TYPE artifact_kind ADD VALUE IF NOT EXISTS 'USER_BRIEF';
ALTER TYPE artifact_kind ADD VALUE IF NOT EXISTS 'GROUNDED_FINDINGS';
ALTER TYPE artifact_kind ADD VALUE IF NOT EXISTS 'AGENT_OUTPUT';
ALTER TYPE artifact_kind ADD VALUE IF NOT EXISTS 'SYSTEM_SPEC';
ALTER TYPE artifact_kind ADD VALUE IF NOT EXISTS 'FLOW_SNAPSHOT';
...
CREATE INDEX IF NOT EXISTS systems_slug_idx ON public.systems (project_id, slug);
...
CREATE TABLE IF NOT EXISTS public.system_artifacts (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id UUID NOT NULL REFERENCES public.projects(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS system_artifacts_system_idx ON public.system_artifacts (system_id);
CREATE INDEX IF NOT EXISTS system_artifacts_kind_idx ON public.system_artifacts (system_id, kind);
...
CREATE TABLE IF NOT EXISTS public.system_evidence (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id UUID NOT NULL REFERENCES public.projects(id) ON DELETE CASCADE,
  system_id UUID NOT NULL REFERENCES public.systems(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS system_evidence_system_idx ON public.system_evidence (system_id);
CREATE INDEX IF NOT EXISTS system_evidence_source_idx ON public.system_evidence (source_type, source_ref);
...
CREATE TABLE IF NOT EXISTS public.system_flow_snapshots (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id UUID NOT NULL REFERENCES public.projects(id) ON DELETE CASCADE,
  system_id UUID NOT NULL REFERENCES public.systems(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS system_flow_snapshots_system_idx ON public.system_flow_snapshots (system_id);
CREATE INDEX IF NOT EXISTS system_flow_snapshots_version_idx ON public.system_flow_snapshots (system_id, version DESC);
...
CREATE TABLE IF NOT EXISTS public.system_node_verifications (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id UUID NOT NULL REFERENCES public.projects(id) ON DELETE CASCADE,
  system_id UUID NOT NULL REFERENCES public.systems(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS system_node_verifications_system_idx ON public.system_node_verifications (system_id);
CREATE INDEX IF NOT EXISTS system_node_verifications_verified_idx ON public.system_node_verifications (system_id, is_verified);
...
DROP POLICY IF EXISTS "Project members can view systems" ON public.systems;
CREATE POLICY "Project members can view systems"
  ON public.systems FOR SELECT
  USING (public.is_project_member(project_id));

DROP POLICY IF EXISTS "Project members can create systems" ON public.systems;
...
DROP POLICY IF EXISTS "Project admins can update systems" ON public.systems;
CREATE POLICY "Project admins can update systems"
  ON public.systems FOR UPDATE
  USING (public.is_project_admin(project_id));

DROP POLICY IF EXISTS "Project admins can delete systems" ON public.systems;
CREATE POLICY "Project admins can delete systems"
  ON public.systems FOR DELETE
  USING (public.is_project_admin(project_id));

-- Artifacts RLS Policies
DROP POLICY IF EXISTS "Project members can view artifacts" ON public.system_artifacts;
CREATE POLICY "Project members can view artifacts"
  ON public.system_artifacts FOR SELECT
  USING (public.is_project_member(project_id));

DROP POLICY IF EXISTS "Project members can create artifacts" ON public.system_artifacts;
...
DROP POLICY IF EXISTS "Project admins can update artifacts" ON public.system_artifacts;
CREATE POLICY "Project admins can update artifacts"
  ON public.system_artifacts FOR UPDATE
  USING (public.is_project_admin(project_id));

DROP POLICY IF EXISTS "Project admins can delete artifacts" ON public.system_artifacts;
CREATE POLICY "Project admins can delete artifacts"
  ON public.system_artifacts FOR DELETE
  USING (public.is_project_admin(project_id));

-- Evidence RLS Policies
DROP POLICY IF EXISTS "Project members can view evidence" ON public.system_evidence;
CREATE POLICY "Project members can view evidence"
  ON public.system_evidence FOR SELECT
  USING (public.is_project_member(project_id));

DROP POLICY IF EXISTS "Project members can create evidence" ON public.system_evidence;
CREATE POLICY "Project members can create evidence"
  ON public.system_evidence FOR INSERT
  WITH CHECK (public.is_project_member(project_id));

DROP POLICY IF EXISTS "Project admins can update evidence" ON public.system_evidence;
CREATE POLICY "Project admins can update evidence"
  ON public.system_evidence FOR UPDATE
  USING (public.is_project_admin(project_id));

DROP POLICY IF EXISTS "Project admins can delete evidence" ON public.system_evidence;
CREATE POLICY "Project admins can delete evidence"
  ON public.system_evidence FOR DELETE
  USING (public.is_project_admin(project_id));

-- Flow Snapshots RLS Policies
DROP POLICY IF EXISTS "Project members can view flow snapshots" ON public.system_flow_snapshots;
CREATE POLICY "Project members can view flow snapshots"
  ON public.system_flow_snapshots FOR SELECT
  USING (public.is_project_member(project_id));

DROP POLICY IF EXISTS "Project members can create flow snapshots" ON public.system_flow_snapshots;
...
DROP POLICY IF EXISTS "Project admins can update flow snapshots" ON public.system_flow_snapshots;
CREATE POLICY "Project admins can update flow snapshots"
  ON public.system_flow_snapshots FOR UPDATE
  USING (public.is_project_admin(project_id));

DROP POLICY IF EXISTS "Project admins can delete flow snapshots" ON public.system_flow_snapshots;
...
DROP POLICY IF EXISTS "Project members can view verifications" ON public.system_node_verifications;
CREATE POLICY "Project members can view verifications"
  ON public.system_node_verifications FOR SELECT
  USING (public.is_project_member(project_id));

DROP POLICY IF EXISTS "Project admins can create verifications" ON public.system_node_verifications;
CREATE POLICY "Project admins can create verifications"
  ON public.system_node_verifications FOR INSERT
  WITH CHECK (public.is_project_admin(project_id));

DROP POLICY IF EXISTS "Project admins can update verifications" ON public.system_node_verifications;
CREATE POLICY "Project admins can update verifications"
  ON public.system_node_verifications FOR UPDATE
  USING (public.is_project_admin(project_id));

DROP POLICY IF EXISTS "Project admins can delete verifications" ON public.system_node_verifications;
...
DROP TRIGGER IF EXISTS set_snapshot_version_trigger ON public.system_flow_snapshots;
...
DROP TRIGGER IF EXISTS update_system_on_artifact_change ON public.system_artifacts;
...
DROP TRIGGER IF EXISTS update_system_on_snapshot_change ON public.system_flow_snapshots;
CREATE TRIGGER update_system_on_snapshot_change
  AFTER INSERT OR UPDATE OR DELETE ON public.system_flow_snapshots
  FOR EACH ROW EXECUTE FUNCTION public.update_system_on_child_change();

DROP TRIGGER IF EXISTS update_system_on_evidence_change ON public.system_evidence;
